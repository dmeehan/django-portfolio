...
class ProjectBase(models.Model):
    ...
    status = models.PositiveSmallIntegerField(choices=STATUS_CHOICES,
        default=STATUS_LIVE, help_text="Only project with live status will be publicly displayed.")
    ...
    def get_next_project(self):
        return self.get_previous_by_completed(status=self.STATUS_LIVE)

    def get_previous_project(self):
        return self.get_next_by_completed(status=self.STATUS_LIVE)
...
class PhysicalMixin(models.Model):
    ...
    def convert_units(self, someUnit):
    ...
    def square_feet(self):
        return self.convert_units(self.UNIT_SQUAREFOOT)

    @property
    def square_meters(self):
        return self.convert_units(self.UNIT_SQUAREMETER)

    @property
    def acres(self):
        return self.convert_units(self.UNIT_ACRE)

    @property
    def hectares(self):
        return self.convert_units(self.UNIT_HECTARE)
    ...
    def save(self, force_insert=False, force_update=False):
        self.area_normalized = self.convert_units(self.UNIT_SQUAREFOOT)
...
class Project(ProjectBase, PhysicalMixin):
    ...
    def get_main_image(self):
        return self.projectvisual_set.filter(main=True)
